...
export const adminSupportService = {
  ...
  async reply(agentId: string, conversationId: string, content: string, attachmentUrls?: string[]) {
    const conversation = await conversationRepository.findById(conversationId);
    if (!conversation) throw new NotFoundError("Conversation");
  ...
  settings: {
    ...
    update: (key: string, value: string, updatedById?: string) => supportSettingRepository.upsert(key, value, updatedById),
  },
};
