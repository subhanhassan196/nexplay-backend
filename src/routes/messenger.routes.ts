...
import { chatAttachmentUpload, chatFileUpload } from "@/middlewares/upload.middleware";
...
// Images, documents and voice notes — returns metadata to attach to a message.
router.post("/files", chatFileUpload, messengerController.uploadFile);
// Arriving via an agent's personal support link.
router.post("/agent-link/:slug", messengerController.claimAgentLink);
