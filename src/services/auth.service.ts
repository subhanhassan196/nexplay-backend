...
export const authService = {
  ...
  async login(
    ...
    // Recorded so a customer can recognise their own account later, and
    // so staff can spot overlapping sign-ins. Deliberately not awaited —
    // a logging failure must never block a valid login.
    void accountSecurityService.recordLogin(user.id, {
      ipAddress: meta.ipAddress,
      userAgent: meta.userAgent,
      success: true,
    });

    // The Activity Center reads AuditLog, so sign-ins have to be written
    // there too — recordLogin only fills the security-specific table.
    void activityService.record({
      actorId: user.id,
